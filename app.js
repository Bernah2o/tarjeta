...
function buildWhatsAppLink(p, extraMessage = '') {
  ...
  const base = `Hola, vi tu tarjeta digital de ${p.company || 'DH2OCOL'}. ${siteLink}`;
...
function initProfile(p) {
  const displayName = (p.name && p.name.trim()) || `${(p.first_name||'').trim()} ${(p.last_name||'').trim()}`.trim() || 'DH2O';
  ...
  const btnTransfer = document.getElementById('btn-pay-transfer'); if (btnTransfer) btnTransfer.href = payLink('Quiero pagar por transferencia.');
  const btnCash = document.getElementById('btn-pay-cash'); if (btnCash) btnCash.href = payLink('Quiero pagar en efectivo.');
  const btnPse = document.getElementById('btn-pay-pse'); if (btnPse) btnPse.href = payLink('Quiero pagar por PSE.');
  const btnCard = document.getElementById('btn-pay-card'); if (btnCard) btnCard.href = payLink('Quiero pagar con tarjeta de crédito.');
  const btnInstr = document.getElementById('btn-pay-instructions'); if (btnInstr) btnInstr.href = payLink('Por favor envíame las instrucciones de pago.');
  ...
  const openWa = document.getElementById('btn-open-whatsapp'); if (openWa) openWa.href = waLink;
  const openMail = document.getElementById('btn-open-mail'); if (openMail) openMail.href = mailLink;

  // Perfil
  const firstEl = document.getElementById('first-name'); if (firstEl) firstEl.textContent = p.first_name || '—';
  const lastEl = document.getElementById('last-name'); if (lastEl) lastEl.textContent = p.last_name || '—';
  const jobEl = document.getElementById('job-title'); if (jobEl) jobEl.textContent = p.job_title || '';
  const companyEl = document.getElementById('company'); if (companyEl) companyEl.textContent = p.company || '—';
  ...
  if (avatarEl) {
    const avatarSrc = p.avatar || 'img/logo_gerente.JPG';
    avatarEl.src = avatarSrc;
    avatarEl.onerror = () => { avatarEl.src = 'img/icon-192.png'; };
  }
  if (brandEl) {
    const brandSrc = p.brand_logo || 'img/icon-192.png';
    brandEl.src = brandSrc;
    brandEl.onerror = () => { brandEl.src = 'img/icon-192.png'; };
  }
  ...
  if (topEmail) { topEmail.textContent = p.email || ''; topEmail.href = p.email ? `mailto:${p.email}` : '#'; }
  const topSite = document.getElementById('top-site');
  if (topSite) { topSite.textContent = p.site || window.location.origin; topSite.href = p.site || window.location.origin; }
  ...
  if (qrThumb) { qrThumb.src = buildQrUrl(siteLink); qrThumb.addEventListener('click', showQr); }
...
function onCopyNequi() {
  ...
  navigator.clipboard.writeText(txt).then(() => {
    ...
    setTimeout(() => document.getElementById('btn-copy-nequi').textContent = 'Copiar', 1500);
  });
}
...
function onCopyWhatsApp() {
  ...
  navigator.clipboard.writeText(txt).then(() => {
    ...
    setTimeout(() => document.getElementById('btn-copy-whatsapp').textContent = 'Copiar', 1500);
  });
}
...
function onCopyMail() {
  ...
  navigator.clipboard.writeText(txt).then(() => {
    ...
    setTimeout(() => document.getElementById('btn-copy-mail').textContent = 'Copiar', 1500);
  });
}
...
async function bootstrap() {
  ...
  const btnCopyWa = document.getElementById('btn-copy-whatsapp'); if (btnCopyWa) btnCopyWa.addEventListener('click', onCopyWhatsApp);
  const btnCopyMail = document.getElementById('btn-copy-mail'); if (btnCopyMail) btnCopyMail.addEventListener('click', onCopyMail);
  const btnVcard = document.getElementById('btn-vcard'); if (btnVcard) btnVcard.addEventListener('click', onSaveVCard);
  const btnShare = document.getElementById('btn-share'); if (btnShare) btnShare.addEventListener('click', onShare);
  const btnQr = document.getElementById('btn-qr'); if (btnQr) btnQr.addEventListener('click', showQr);
  const btnQrCard = document.getElementById('btn-qr-card');
  if (btnQrCard) btnQrCard.addEventListener('click', showQr);
  const btnCloseQr = document.getElementById('btn-close-qr'); if (btnCloseQr) btnCloseQr.addEventListener('click', closeQr);
  const btnSaveQr = document.getElementById('btn-save-qr'); if (btnSaveQr) btnSaveQr.addEventListener('click', saveQrImage);
